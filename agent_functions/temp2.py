...
def _compute_predictor_priors(class_priors, world_size):
    """Computes the probability of a breeze in each location."""
    predictor_priors = np.zeros((world_size, world_size))  # Breeze probs
...
def _compute_likelihoods(percept_loc, pred_loc, B_pit_removed, world_size, percept):
    ...
    if percept.get_breeze():
        ...
        if pred_loc in adj_locs:
            ...
            px, py = percept_loc
            return 1 - B_pit_removed[px][py]


def _compute_posteriors(pred_loc, P, percept_loc, B, B_pit_removed, world_size, percept):
    """Computes P(P_xy|B_percept_loc), using Bayes Rule"""
    if percept.get_breeze():
        pcpt_x, pcpt_y = percept_loc
        likelihoods  = _compute_likelihoods(percept_loc, pred_loc, B_pit_removed, world_size, percept)
        class_priors = P[pred_loc[0]][pred_loc[1]]
        predictor_priors = B[pcpt_x, pcpt_y]
        if predictor_priors == 0:
            posteriors = 0
        else:
            posteriors = (likelihoods * class_priors) / predictor_priors  # Bayes Rule
        return posteriors
    else:
        pcpt_x, pcpt_y = percept_loc
        likelihoods      = _compute_likelihoods(percept_loc, pred_loc, B_pit_removed, world_size, percept)
        class_priors     = P[pred_loc[0]][pred_loc[1]]
        predictor_priors = 1 - B[pcpt_x, pcpt_y]
        if predictor_priors == 0:
            posteriors = 0
        else:
            posteriors  = (likelihoods * class_priors) / predictor_priors  # Bayes Rule
...
class AgentFunction:
    ...
    def __init__(self):
        ...
        self.P_is_known = np.zeros((self.WORLD_SIZE, self.WORLD_SIZE))  # whether we know if a Pit exists in each square
        self.W_is_known = np.zeros((self.WORLD_SIZE, self.WORLD_SIZE))   # whether we know if a Wumpus exists in each square
        self.G_is_known = np.zeros((self.WORLD_SIZE, self.WORLD_SIZE))   # whether we know if the exists in each square
        self.P[0][0] = 0
        self.W[0][0] = 0
        self.P_is_known[0][0] = 1
        self.W_is_known[0][0] = 1
    ...
    def _update_P(self, percept):
        ...
        prior_P_removed = _recompute_probs_after_removal(prior_P)  # Pit probabilities with 1 pit removed (see future computation in _compute_likelihoods())
        B_pit_removed = _compute_predictor_priors(prior_P_removed, self.WORLD_SIZE)
        for x in range(self.WORLD_SIZE):
            for y in range(self.WORLD_SIZE):
                pred_loc = [x, y]
                self.P[x][y] = _compute_posteriors(pred_loc, prior_P, percept_loc, B, B_pit_removed, self.WORLD_SIZE, percept)
    ...
    def _choose_action(self):
        ...
        if self.G_is_known[x][y] == 1 and self.G[x][y] == 1:
            return Action.GRAB
